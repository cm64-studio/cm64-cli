...
/**
 * Remove a single cached file entry. Used when the server has deleted a file
 * the local cache still knows about, so subsequent pushes don't keep flagging
 * the same ghost.
 */
export function removeCachedFile(projectId, filePath) {
  const fp = cacheFilePath(projectId, filePath);
  if (existsSync(fp)) {
    try { rmSync(fp, { force: true }); } catch (_) { /* best effort */ }
  }
}
